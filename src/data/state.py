...
class StateManager:
        ...
        try:
            with Session(self.engine) as session:
                for candle in candles:
                    ...
                    existing = session.query(OHLCVCandle).filter_by(
                        symbol=symbol,
                        timeframe=timeframe,
    ...
    def get_ohlcv_candles(
        ...
        try:
            with Session(self.engine) as session:
                ...
                candles = query.order_by(OHLCVCandle.timestamp.asc()).limit(limit).all()
    ...
    def get_candle_count(self, symbol: str, timeframe: str) -> Dict[str, Any]:
        ...
        try:
            with Session(self.engine) as session:
                count = session.query(OHLCVCandle).filter_by(
                    symbol=symbol,
                    timeframe=timeframe
                ).count()
                ...
                first_candle = session.query(OHLCVCandle).filter_by(
                    symbol=symbol,
                    timeframe=timeframe
                ).order_by(OHLCVCandle.timestamp.asc()).first()
                
                last_candle = session.query(OHLCVCandle).filter_by(
                    symbol=symbol,
                    timeframe=timeframe
                ).order_by(OHLCVCandle.timestamp.desc()).first()
                ...
                return {
                    ...
                    "first_date": datetime.fromtimestamp(first_candle.timestamp/1000).isoformat() if first_candle else None,
                    "last_date": datetime.fromtimestamp(last_candle.timestamp/1000).isoformat() if last_candle else None,
                    "storage_mb": round(storage_mb, 2),
                    "estimated_years": round(count / (365 * 24 * 60), 2) if timeframe == "1m" else None
                }
        except SQLAlchemyError as e:
